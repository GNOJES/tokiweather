...
private fun WeatherWidgetContent(
    ...
    // 3×2 Large 위젯 감지: 세로 공간이 충분하면 세로 2단 레이아웃 적용
    val isLarge = widgetHeight >= 110.dp
    ...
    if (isLarge) {
        LargeWidgetContent(
            ...
            subTextColorProvider = subTextColorProvider,
...
/**
 * 3×2 Large 위젯 레이아웃 (노바런처 등 세로형)
 * - 상단: 오늘 날씨 (큰 아이콘 + 기온 + 미세먼지 + 지역명)
 * - 하단: 내일/모레 예보 (아이콘 + 기온 + 강수확률 바)
 */
@Composable
private fun LargeWidgetContent(
    ...
    widgetWidth: Dp,
    widgetHeight: Dp,
    widgetBgColor: ComposeColor,
    textColorProvider: ColorProvider,
    subTextColorProvider: ColorProvider,
    mainTextColor: ComposeColor,
    fontScale: Float,
    hasData: Boolean
) {
    // 반응형 크기 계산
    val isNarrow = widgetWidth < 155.dp
    val isTall = widgetHeight >= 160.dp

    val horizPadding = if (isNarrow) 8.dp else 12.dp
    val vertPadding = if (isTall) 8.dp else 6.dp

    val todayIconSize = if (isNarrow) 36.dp else 44.dp
    val todayTempSize = if (isNarrow) 20 else 24
    val todayPmSize = if (isNarrow) 9 else 10
    val locNameSize = if (isNarrow) 9 else 10
    val forecastIconSize = if (isNarrow) 20.dp else 24.dp
    val forecastTempSize = if (isNarrow) 9f else 10f
    val forecastLabelSize = if (isNarrow) 8f else 9f
    val popBlockSize = if (isNarrow) 3.5.dp else 4.dp
    val forecastSpacer = if (isNarrow) 4.dp else 8.dp
        ...
        if (!hasData) {
            ...
            Row(
                ...
                Image(
                    ...
                    contentDescription = "위치",
                    colorFilter = ColorFilter.tint(subTextColorProvider),
                    modifier = GlanceModifier.size(if (isNarrow) 8.dp else 10.dp)
                )
                Spacer(modifier = GlanceModifier.width(1.5.dp))
                Text(
                    text = weather.locationName,
            ...
            Spacer(modifier = GlanceModifier.height(if (isTall) 4.dp else 2.dp))
            ...
            Spacer(modifier = GlanceModifier.height(if (isTall) 6.dp else 4.dp))
